...
task_writing_survey_id <- surveys$id[3]
...
# Prior to CSV generation: handle cases where we wrote new writeups but things already got rated
Basically, we want to generate new indices for tasks where we're now explicitly saving a new version for.
- If something hasn't been rated yet, we can go ahead and update the writeup. It's fine.
- If something is already rated, we need to generate a new entry (new row on the CSV) for it so that we can store it as a separate ID
- So this means that, in order to create a stable index:
     * we will manually set the ID to a known particular ID
     * the value of the ID will be N+1, where N is the number of tasks that are currently being actively rated in the pipeline
     * each time this happens again, we'll have to set that manual ID again, but this will allow that ID to remain consistent throughout the process.
```{r}
index_in_survey_1 <- 26
# Putting Food into Categories and Whac a Mole
new_tasks <- c("Putting food into categories-2", "Whac-A-Mole-2")
all_tasks <- c(df.survey_results$`task-name`[1:index_in_survey_1], new_tasks, df.survey_results$`task-name`[(index_in_survey_1+1):length(df.survey_results$`task-name`)])
```

We will use `all_tasks` moving forward to ensure both the original and the manually-added ones show up and are consistent!
...
Get survey data
```{r}
task_mapping_survey_id <- surveys$id[4]
...
# TODO - explore shifting this to blobs instead
```{r}
attributes(df.task_map)$column_map

req <- httr::GET("https://api.github.com/repos/Watts-Lab/task-mapping/git/trees/master?recursive=1")
# jsonlite::fromJSON(content(req, "text"))$tree %>% 
...
inner_join(categorical, numeric, by = "task_name") %>% write_csv('../task_map_new.csv')
```
...
Set up to collect the task map while we run the Krippendorf loop -- that way we can leverage the data where we drop the 'worst' users
```{r}
...
Leveraging the updated Krippendorf's code from IRR
```{r}
kripp_alpha_per_question <- data.frame(
  question_name = c(),
  #num_tasks = c(),
...
for (i in 1:length(main_questions)) {
  ...
  kripp_alpha_matrix <- df.all_ratings_raw %>%
    ...
    left_join(df.task_map_summary %>% select(task_name, n_labels)) %>% # get n_labels
    pivot_wider(
      names_from = c(IPAddress),
      values_from = c(value),
      values_fn = list
    ) %>%
    column_to_rownames("task_name") %>%
    ungroup() %>%
    select(-c(task_num, question_name, n_labels)) %>%
    mutate(across(everything(), as.character)) %>%
  ...
  if (q_name == 'Q2intel_manip_1' |
    ...
    num_responses <- rowSums(!is.na(kripp_alpha_matrix))
    ...
    pct_deviations <- num_deviations/num_responses

    ### DROP THE N WORST PEOPLE
    N_Worst <- 1 # set the number of "worst" that we want to drop
    worst_indices <- tail(order(pct_deviations), N_Worst)
...
```{r}
df.task_map_from_filtered <- df.task_map_from_filtered %>%
  drop_na()
```
...
p <- combined_data %>%
  ggplot(aes(
    ...
    fill = cluster
                                            ...
                                            alpha = 0.05) +
  geom_point(
    data = subset(combined_data, task_name %in% implemented_tasks),
    aes(
      x = PC1,
      y = PC2,
    )
...
#ggsave(plot = p, filename = 'task-map-unreliable-dropped-10-tasks-highlighted.png')
#ggsave(plot = p, filename = 'task-map-standard.png')
#ggsave(plot = p, filename = 'task-map-unreliable-dropped.png')
fviz_eig(pca)
...
ggsave(plot = p.old, filename = 'task-map-OLD.png')
fviz_eig(pca.old)
```
