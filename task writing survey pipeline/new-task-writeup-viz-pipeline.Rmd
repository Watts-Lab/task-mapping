...
filter(grepl("task\ writing\ survey\ pipeline/new-task-writeup-viz-pipeline.Rmd", path))
...
df.task_map_format <- data.frame(matrix(ncol = 4, nrow = 0))
colnames(df.task_map_format) <- c('Task Name', 'Task Writeup', 'GitHub Blob URL', 'Generating Script URL')
...
for(i in 1:length(all_tasks)){
  ...
  new_row = data.frame(
    ...
    'GitHub Blob URL' = blob_url,
    'Generating Script URL' = thisscript.url
  )
