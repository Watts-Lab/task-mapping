...
df.all_ratings_raw <- df.task_map %>%
  ...
  mutate(
    ...
    question_name = substring(question_name,2)
  ) %>% 
  drop_na() %>%
  group_by(task_num) %>%
...
df.main_questions_summary <- df.all_ratings_raw %>%
  ...
  summarize(mean_rating = mean(value),
            ...
            agreement = agree(t(value))$value
            )

# krippendorfs alpha across all values with 2 labels
df_multilabeler <- df.main_questions_summary %>%
  filter(n_labels == 2)

lbls_matrix <- matrix(nrow = 0, ncol = 2)
for(i in 1:nrow(df_multilabeler)){
  all_values <- df_multilabeler[i,]$all_values
  lbls <- as.numeric(str_split(all_values, '&')[[1]])
  lbls_matrix <- rbind(lbls_matrix,lbls)                 
}
kripp.alpha(t(lbls_matrix))
```
