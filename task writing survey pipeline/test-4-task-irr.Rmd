...
cat_raw <- df.all_ratings_raw %>%
  ...
  cat_raw %>% filter(question_name %in% main_questions[1:11]) %>% ggplot(
    aes(
      x = value,
    )
  ) + 
  facet_grid(rows = vars(task_name),
             cols = vars(question_name)) +
  geom_bar()
```

```{r}
  cat_raw %>% filter(question_name %in% main_questions[12:23]) %>% ggplot(
    aes(
      x = value,
    )
  ) + 
