...
```{r}
# transform the data to convert everything to ints
df.name_lookup = data.frame(
  name = unique(df.all_ratings_raw$name),
  name_index = seq(1:length(unique(df.all_ratings_raw$name)))-1
 )

df.task_index = data.frame(
  task_num = unique(df.all_ratings_raw$task_num),
  task_index = seq(1:length(unique(df.all_ratings_raw$task_num)))-1
)

df.question_index = data.frame(
  question_name = main_questions,
  question_index = seq(1:length(main_questions))-1
)
...
df.all_ratings_raw %>%
  ...
  left_join(df.name_lookup, by = "name") %>%
  filter(
    question_name %in% main_questions
  ) %>%
  left_join(df.question_index, by = "question_name") %>%
  left_join(df.task_index, by = "task_num") %>%
  select(task_index, question_index, name_index, value) %>% drop_na() %>% write_csv('within-lab-4-task-irr.csv')
```
...
task_based_summary
```
This is the large KA block, that also doubles as doing 
