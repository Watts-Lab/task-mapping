...
tasks = c("Word completion given starting letter", "Graph coloring task", "Shopping plan")
...
# for(i in 1:length(tasks)){ # separately do this for every task --- questions are nested by task
  ...
  A <- df.karger %>% filter(task_name == tasks[i]) %>% select(-task_name) %>%
