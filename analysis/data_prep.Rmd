...
clean_responses <- raw_responses %>%
  ...
  mutate(
    ...
    user = trimws(
      gsub("  "," ",
      gsub("( and |, |&)"," \\| ",
      gsub("(Subramaniam|Bottcher|Sampath|Cullen|Balasubramanian)","",
      gsub("sarikasu","Sarika",
           gsub("(katelynbottcher|Katelynn)","Katelyn",
    ...
    name_clean = trimws(
      gsub("  ", " ",
           gsub("finding the maximum","find the maximum",
           gsub("recall associations","recall association",
  ...
  left_join(tasks %>% mutate(name_clean = trimws(
    gsub("  ", " ", 
         sub("task","",
        gsub("various versions","",
         gsub("hidden profile","",
...
unrecognised_tasks <- clean_responses %>% select(task,name_clean) %>% filter(is.na(task)) %>% arrange(name_clean)
```
...
autoplot(pca,
         ...
         shape = FALSE,
         label.size = 3) +
  labs(title = "PCA of mapped tasks (across all features)", x = "", y = "") + 
  ggpubr::theme_pubclean()
ggsave("Mapped_task_PCA.pdf",
       width = 7,
       height = 5) 
```
