...
clean_responses <- raw_responses %>%
  ...
  ))) %>% select(name, name_clean,`phase 1 editor`,`phase 2 editor`,phase, source, implemented)) %>%
  mutate(task = name,
...
```{r}
# TODO output table with individual properties + source + instructions
clean_responses %>% 
  filter(stage == "agreement") %>%
  select(task,individual_theoretical,individual_practical,source,implemented)
```
...
agreed_responses <- clean_responses %>%
  filter(stage == "agreement") %>%
  select(-stage, -user, -updatedAt, -editors, -name, -name_clean, -phase, -source) %>%
  distinct() %>%
  arrange(task)
