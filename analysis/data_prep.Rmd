...
clean_responses <- raw_responses %>%
  ...
  ))) %>% select(name, name_clean,`phase 1 editor`,`phase 2 editor`,phase)) %>%
  mutate(task = name,
...
agreed_responses <- clean_responses %>%
  ...
  select(-stage,-user,-updatedAt,-editors,-name,-name_clean,-phase) %>%
  distinct() %>%
  arrange(task)
...
clean_responses %>% 
  filter(stage != "",editors != " -> ") %>% 
  group_by(task,stage) %>% 
  summarise(mappers=paste(user, collapse = ' + '),editors = first(editors)) %>% 
  pivot_wider(names_from = stage, values_from = mappers,values_fill="") %>% 
  filter(agreement=="") %>%
  select(-agreement,-`1`,-`2`) %>%
  rename(mappers_so_far = individual) %>%
  arrange(task)

clean_responses %>% group_by(task,stage) %>% summarise(editors = first(editors)) %>% arrange(task,stage) %>% group_by(task) %>% summarise(stage = first(stage)) %>% write_csv("../tasks.csv")
```
