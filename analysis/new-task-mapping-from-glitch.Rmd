...
quantile(question_based_summary$mean_agreement, c(0.025, 0.9725))
```
...
# 
# for(task_blob in task_based_summary$task_blob_url){
#   long_tbl_q_ans <- df.mapping.raw %>% select(c(main_questions, task, task_blob_url, user)) %>%
#   select(-continuous_questions) %>%
...
#                      "No" = 0,
#                      "Yes" = 1,
#                    )) %>% filter(task_blob_url == task_blob) %>%
#     pivot_wider(names_from = question_name, values_from = value)
# 
#   task_name <- long_tbl_q_ans$task[1]
#   
#   ka <- long_tbl_q_ans %>%
...
#   )
#   
#   kripp.alpha.by.task <- rbind(kripp.alpha.by.task, new_row)
# }
# 
...
KA by question
```{r}
# kripp.alpha.by.question <- data.frame(matrix(nrow = 0, ncol = 2))
# 
# for(q in question_based_summary$question_name){
#   long_tbl_q_ans <- df.mapping.raw %>% select(c(main_questions,task_blob_url, user)) %>%
#   select(-continuous_questions) %>%
#   pivot_longer(cols = -c(task_blob_url, user), names_to = "question_name") %>%
#   mutate(value = recode(value, 
#                      "Mental" = 0,
#                      "Physical" = 1,
#                      "Not applicable or not answerable based on the task description (Please Elaborate Below.)" = 3,
#                      "No" = 0,
#                      "Yes" = 1,
#                    )) %>% filter(question_name == q) %>%
#     pivot_wider(names_from = task_blob_url, values_from = value)
# 
#   ka <- long_tbl_q_ans %>%
#   select(-c(user, question_name)) %>% 
#   as.matrix() %>% kripp.alpha("ordinal")
#   
#   new_row = data.frame(
#     question = q,
...
#   )
#   
#   kripp.alpha.by.question <- rbind(kripp.alpha.by.question, new_row)
# }
# 
# kripp.alpha.by.question %>% arrange(desc(k.alpha))
```
...
p <- combined_data %>%
  ...
  # geom_label(
  #   data = subset(combined_data, task %in% james_set),
  #   aes(
  #     x = PC1,
  #     y = PC2,
  #     label = task ,
