...
```{r}
t# df.mapping.raw %>%
#   filter(Q9divisible_unitary == "Not applicable or not answerable based on the task description (Please Elaborate Below.)") %>%
#   select(Q9divisible_unitary_elaboration, task) %>% View()
```
...
# NOTE -- replacing with 5 for better visual purposes
...
```{r}
# responses_clean_ind <- df.mapping.raw %>%
#   select(c(main_questions, task, task_blob_url, user)) %>%
#   select(-continuous_questions) %>%
#   pivot_longer(cols = -c(task,task_blob_url, user), names_to = "question_name") %>%
#   mutate(value = recode(value, 
#                      "Mental" = 0,
#                      "Physical" = 1,
#                      "Not applicable or not answerable based on the task description (Please Elaborate Below.)" = 3,
