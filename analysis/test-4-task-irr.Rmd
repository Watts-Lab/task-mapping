...
#task_mapping_irr_survey_id <- (surveys %>% filter(name == "Task Mapping IRR on 4 Tasks"))$id #RA version of the survey
task_mapping_irr_survey_id <- (surveys %>% filter(name == "[MTURK VERSION] Task Mapping IRR on 4 Tasks"))$id #MTurk version of the survey
```
...
Look at duration
```{r}
durations <- df.task_map %>%
  transmute(duration = `Duration (in seconds)`/60)

median(durations$duration)/4
```
...
if(task_mapping_irr_survey_id == surveys$id[2]){
  ...
  select(task_name, question_name, name, value) %>%
    write_csv('linnemilymabdullah_ground_truth.csv')
}
```
...
#df.karger %>% write_csv('df_for_karger.csv')
```
...
df.all_ratings_raw %>%
  ...
  arrange(question_name) %>% write_csv("./task_mapping_feedback-mturk.csv")
```
