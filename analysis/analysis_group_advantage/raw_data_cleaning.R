...
load_CSVs <- function(pattern, rename = TRUE) {
  ...
  if (rename) {
    ...
    if ("_id" %in% names(data)) {
      # Use rlang::sym to convert the character name into a symbol for `:=`
      data <- data |> rename(!!rlang::sym(adjusted_pattern) := `_id`)
    }
...
ggsave("../../outputs/figures/figure_2.pdf", plot = figure_2, height = 14, width = 14)
ggsave("../../outputs/figures/figure_2.png", plot = figure_2, height = 14, width = 14)
