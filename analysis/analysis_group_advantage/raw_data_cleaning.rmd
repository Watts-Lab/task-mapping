...
synergy_data_for_prediction |> 
  ...
  mutate(mcgrath_type = if_else(is.na(mcgrath_type), "Uncategorized", mcgrath_type)) |> 
  mutate(mcgrath_type = factor(mcgrath_type, levels = mcgrath_category_levels)) |>
  mutate(type = 1) |> 
  pivot_wider(names_from = mcgrath_type, values_from = type, values_fill = 0) |>
  rename_with(~ paste0(.x, "_cat"), .cols = -all_of(base_columns)) |>
  write_csv("../../outputs/processed_data/condition_level_group_advantage_with_ivs_and_categories.csv")
```
