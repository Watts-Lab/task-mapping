...
export default class Game2 extends React.Component {
    ...
    resetCanvas() {
        ...
        var horizontalBar = Matter.Bodies.rectangle(330, 120, 220, 5, 
            {
                isStatic: true, 
                friction: 0.4,
                render: {
        ...
        var rightTilted = Matter.Bodies.rectangle(350, 63, 150, 5, 
            {
                isStatic: true, 
                angle: 40, 
                render: {
        ...
        var leftCup = Matter.Bodies.rectangle(150, 320, 70, 5, 
            {
                angle: -30, 
                render: {
                    fillStyle: "coral"
        ...
        var rightCup = Matter.Bodies.rectangle(210, 320, 70, 5,
            {
                angle: 30, 
                render: {
                    fillStyle: "coral"
        ...
        var bottom = Matter.Bodies.rectangle(180, 353, 60, 5, 
            {
                frictionStatic: 10,
                render: {
                    fillStyle: "coral"
        ...
        var blueBall = Matter.Bodies.circle(180, 340, 20, 
            {
                friction: 0,
                render: {
        ...
        var otherLeftCup = Matter.Bodies.rectangle(245, 100, 35, 5, 
            {
                angle: 30, 
                render: {
                    fillStyle: "gold"
        ...
        var otherRightCup = Matter.Bodies.rectangle(275, 100, 35, 5,
            {
                angle: -30, 
                render: {
                    fillStyle: "gold"
        ...
        var otherBottom = Matter.Bodies.rectangle(260, 85, 30, 5, 
            {
                frictionStatic: 0.01,
                render: {
                    fillStyle: "gold"
        ...
        var otherCup = Matter.Body.create(
            {
                parts: [otherLeftCup, otherRightCup, otherBottom], 
                mass: 5,
                inverseMass: 0.2,
            })
        var greenBall = Matter.Bodies.circle(255, 105, 10, 
            {
                friction: 0.4,
                render: {
        ...
        Matter.World.add(engine1.world, [ground, horizontalBar, rightTilted, cup, blueBall, greenBall, otherCup]);
        Matter.Runner.run(engine1);
        Matter.Render.run(render1);

        this.setState({
            collision: Matter.Collision.create(blueBall, greenBall)
        })
    }
    ...
    submit() {
        ...
        if (this.state.collision !== null) {
            const ballA = this.state.collision.bodyA.position 
            const ballB = this.state.collision.bodyB.position 
            const minBallX = Math.min(ballA.x, ballB.x)
            const maxBallX = Math.max(ballA.x, ballB.x)
            const minBallY = Math.min(ballA.y, ballB.y)
            const maxBallY = Math.max(ballA.y, ballB.y)
            const ballOneRadius = this.state.collision.bodyA.circleRadius 
            const ballTwoRadius = this.state.collision.bodyB.circleRadius
            if (minBallX + ballOneRadius + ballTwoRadius >= maxBallX && minBallY + ballOneRadius + ballTwoRadius >= maxBallY) {
        ...
        return (
            <div style={{marginLeft: "50px", marginTop: "50px"}}>
                ...
                style={{
                    fontSize: "18px",
                    maxWidth: "80rem"
                }}> In this task, your goal is to make the green ball touch the blue ball by adding a larger ball 
                    (about 2x the size of the blue ball) to the canvas. To do so, please use your mouse to click on 
                    the location where you want the ball to appear.
                </p>
                <h3
                style={{                
                    fontSize: "25px",
