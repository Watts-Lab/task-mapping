...
# effort_physical + effort_mental == 6


# TODO - add effort_physical and effort_mental
# and see the *distribution* of sums
...
filter_for_agreement <- function(data_table){
  ...
  filter(effort_physical+effort_mental == TOTAL_SCORE)
...
effort_disagreement<- numerical_completed_tasks %>%
  filter(effort_physical+effort_mental != TOTAL_SCORE)%>%
  select(c(task_names, effort_physical, effort_mental)) %>%
  pivot_longer(c(effort_physical, effort_mental), names_to = "question_name") %>%
  rename(task = task_names) %>%
  left_join((clean_responses %>% filter(stage == "agreement")), by = "task") %>%
  select(c(task, user, question_name, value)) %>%
   pivot_wider(names_from = "question_name") %>%
  mutate(
    question1 = "effort_physical",
    question2 = "effort_mental"
  ) %>%
  rename(
    answer1 = effort_physical,
    answer2 = effort_mental,
    raters = user
  ) %>%
  select(c(task, raters, question1, answer1, question2, answer2))
...
inconsistent_questions <- goal_disagreement %>% rbind(options_disagreement) %>%
  rbind(outcome_disagreement) %>% rbind(individual_disagreement) %>% 
  rbind(submission_disagreement) %>% rbind(effort_disagreement)
  

write.csv(inconsistent_questions,"./analysis_experiments/task_mapping_issues.csv")

inconsistent_questions %>% select(task) %>% unique()
```
...
## Get some distributions of these sums
```{r}
numerical_tasks_with_sums <- numerical_completed_tasks %>%
  mutate(
    options_sum = options_independent+options_dependent,
    outcome_sum = outcome_certain+outcome_probablistic,
    individual_sum = individual_theoretical+individual_practical,
    submission_sum = submission_together + submission_separate,
    effort_sum = effort_physical+effort_mental
  )

ggplot(numerical_tasks_with_sums, aes(
  x = options_sum
)) +
  geom_histogram(bins=7)+
  xlab("options_independent+options_dependent")

ggplot(numerical_tasks_with_sums, aes(
  x = submission_sum
)) +
  geom_histogram(bins=7)+
  xlab("submission_together + submission_separate")

ggplot(numerical_tasks_with_sums, aes(
  x = individual_sum
)) +
  geom_histogram(bins=7)+
  xlab("individual_theoretical+individual_practical")

ggplot(numerical_tasks_with_sums, aes(
  x = outcome_sum
)) +
  geom_histogram(bins=7)+
  xlab("outcome_certain+outcome_probablistic")

ggplot(numerical_tasks_with_sums, aes(
  x = effort_sum
)) +
  geom_histogram(bins=7)+
  xlab("effort_physical+effort_mental")
```
