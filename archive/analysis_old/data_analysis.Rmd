...
responses_clean_ind <- clean_responses %>%
  ...
  select(c(task, user,
        ...
        generate_ideas,generate_creativity,resolve_opinion,resolve_perspectives,
        resolve_information,resolve_data, 
        conflict_cooperation,conflict_interests, 
        external_adversary,external_standard,
        solution_valid, solution_optimal, solution_objectivity,
        task_divisibility, task_specialization,
        measure_easy,measure_immediate)) %>%
...
```{r}
responses_clean_ind %>% ungroup() %>% select(task) %>% unique()
```
...
g <- arrangeGrob(p1,p2,p3,p4,p5,p6,p7,p8, nrow = 4)
