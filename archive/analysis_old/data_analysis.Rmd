...
task_response_summary <- responses_clean_ind %>%
  ...
  # set to binary
  mutate(value_scaled = ifelse(value !=3 , ifelse(value<3,0,1), NA)) %>%
  drop_na() %>%
   summarize(mean_rating = mean(value, na.rm = T),
            agreement_scaled = ifelse(abs(value_scaled-mean(value_scaled))==0, 1, 0),
  ...
  summarize(
    ...
    mean_agreement = mean(agreement, na.rm = T),
    mean_agreement_scaled = mean(agreement_scaled, na.rm = T)
  ) %>%
...
quantile(question_response_summary$mean_agreement, c(0.025, 0.9725), na.rm = T)
...
A <- averaged_by_task %>%
  ...
  labs(title = "A") +
  annotate("text", x = 1.5, y = 4, size = 10, label = paste("r =", 
                                                            toString(round(cor(averaged_by_task$generate_ideas, averaged_by_task$generate_creativity, use="complete.obs"),3)))) +
...
B <- averaged_by_task %>%
  ...
  labs(title = "B") +
  annotate("text", x = 1.5, y = 4, size = 10, label = paste("r =",
                                                            toString(round(cor(averaged_by_task$goal_standard, averaged_by_task$goal_optimize, use="complete.obs"),3)))) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.title = element_text(size=30, face="bold"),
...
C <- averaged_by_task %>%
  ...
  labs(title = "C") +
  annotate("text", x = 1.5, y = 4, size = 10, label = paste("r =",
                                                          toString(round(cor(averaged_by_task$solution_objectivity, averaged_by_task$solution_optimal, use="complete.obs"),3)))) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        plot.title = element_text(size=30, face="bold"),
        legend.text = element_text(size=25))
...
ggsave("corrplot_related_questions.png", g)
```
