...
All data originates from the authors' own data collection. 
...
Both R and Python are required to reproduce this package.
...
Relevant analyses associated with each of the two components can be reproduced by running commands via the master script (`analysis/run_master.py`).
...
```
  ...
  # run both
  python analysis/run_master.py --steps task_space,group_advantag
```
