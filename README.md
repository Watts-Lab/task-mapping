...
Data from Task Map annotation are at the top level of the folder; data from the Group Advantage experiment, which took place in three waves, are available in subfolders `Wave 1 data/`, `Wave 2 data/`, and `Wave 3 dat/`, respectively.
...
The `rating pipelines` subfold documents the process of recruiting and training humans to annotate the Task Map (our repository of 102 tasks, annotated along 24 quantitative dimensions by raters recruited from Amazon Mechanical Turk). These materials may be useful to future researchers wishing to reproduce our rating pipeline:

- `rater_training` contains the Qualtrics survey used to instruct raters on how to annotate the tasks.
- `resources_for_rating` contains data used in the task rating process, including the questions and exact descriptions provided to the raters.
- `survey_workflows` contains code associated with grading the pre-test for filterings raters, as well as the panel of rater comprising our final pool.
- `writeup to html pipeline` contains code and materials associated with translating the written task summaries (which are stored in `.md` format in the `task_writeups` folder) into html files, which was the format in which they were displayed to raters.

#### `multi-task-empirica` subfolder

TODO - this needs to be added as a submodule!
