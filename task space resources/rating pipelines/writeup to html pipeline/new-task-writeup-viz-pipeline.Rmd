...
# Pull in Survey Data
```{r}
# SECURITY: Do not commit API keys. Provide QUALTRICS_API_KEY in your environment before running.
...
for(i in 1:length(edited_tasks)){
  ...
  # Replace the quotes in the name
  ...
  # Replace special characters in the main text
...
Remove all other summaries
- This ensures that any summary that had its name changed after being initially submitted will be removed from the system.
```{r}
for(i in 1:length(df.blobs$path)){
  file_path <- df.blobs$path[i]
  if(!(file_path %in% df.task_map_format$path)){
    file.remove(paste0("../",file_path))
  }
}
```
