...
calculateFleissKappa = function(dataTable, rowGroup, colPattern) {
  ...
  apply(rowGroups,1,function(group){
    ...
    return(apply(cols,1,function(column){
      ...
      return(kappam.fleiss(t(colTable))$value)
    }))
  })
}
...
testData = questionData %>% 
  filter(Task == "Sudoku") %>%
  select(contains("intellectiv"),Task,WorkerId) %>% 
  spread(WorkerId,`intellective - Categorize this task`)

as.matrix(as.matrix(testData))
kripp.alpha(t(as.matrix(testData)))
kappam.fleiss(as.matrix(testData))
```
